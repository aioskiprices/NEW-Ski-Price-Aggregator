...
def get_ski_info_bakerst(url):
    """
    Scrapes the product name and price from a Baker Street Snow product page.

    Args:
        url (str): The URL of the Baker Street Snow product page.
    ...
    try:
        ...
        # ----- 1) Product name -----
        name_element = soup.find('h1', class_='product-title h5')
        product_name = name_element.get_text(strip=True) if name_element else "Name not found"

        # ----- 2) Price -----
        price_container = soup.find('div', class_='product-info__price')
        if price_container:
            price_element = price_container.find('strong', class_='price__current')
        ...
        if product_name != "Name not found":
            try:
                ...
                for ski in data['skis']:
                    if ski['site_name'] == 'Baker Street' and ski['site_url'] == url:
