...
def get_ski_info_blackcrows(url):
    """
    Scrapes the product name and price from a Black Crows product page.

    Args:
        url (str): The URL of the Black Crows product page.
    ...
    try:
        ...
        # ----- 2) Price -----
        price_container = soup.find('div', class_='product-details-prices')
        if price_container:
            price_element = price_container.find('span', class_='sales')
        ...
        if product_name != "Name not found":
            try:
                ...
                for ski in data['skis']:
                    if ski['site_name'] == 'Black Crows' and ski['site_url'] == url:
