...
def get_ski_info_skihaus(url):
    """
    Scrapes the product name and price from a Ski Haus product page.

    Args:
        url (str): The URL of the Ski Haus product page.
    ...
    try:
        ...
        # ----- 1) Product name -----
        name_element = soup.find('h1', class_='productView-title')
        product_name = name_element.get_text(strip=True) if name_element else "Name not found"

        # ----- 2) Price -----
        price_element = soup.find('span', {'data-product-price-without-tax': True, 'class': 'price price--withoutTax'})
        ...
        if product_name != "Name not found":
            try:
                ...
                for ski in data['skis']:
                    if ski['site_name'] == 'Ski Haus' and ski['site_url'] == url:
