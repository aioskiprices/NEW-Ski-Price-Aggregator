...
def get_ski_info_gearx(url):
    """
    Scrapes the product name and price from a GearX product page.

    Args:
        url (str): The URL of the GearX product page.
    ...
    try:
        ...
        # ----- 1) Product name -----
        name_element = soup.find('h1', class_='text-gray-900 page-title title-font')
        if name_element:
            name_span = name_element.find('span', class_='base')
            product_name = name_span.get_text(strip=True) if name_span else "Name not found"
        else:
            product_name = "Name not found"

        # ----- 2) Price -----
        price_container = soup.find('div', class_='final-price inline-block')
        if price_container:
        ...
        if product_name != "Name not found":
            try:
                ...
                for ski in data['skis']:
                    if ski['site_name'] == 'GearX' and ski['site_url'] == url:
